...
export function useDragDrop(
  ...
  const startDrag = (itemId: string, parentId: string, e: PointerEvent) => {
    ...
    let lastClientY = 0;
    let lastClientX = 0;
    // Cache the scroll container to avoid repeated querySelector
    ...
    const rafLoop = () => {
      ...
      if (!scrollContainer) {
        scrollContainer = document.querySelector('.tree-view');
      }

      if (scrollContainer) {
        // Read phase: get container bounds
        ...
        if (lastClientY < rect.top + margin) {
          speed =
            -maxSpeed * (1 - Math.max(0, lastClientY - rect.top) / margin);
      ...
      // Update drop target — getBoundingClientRect here is unavoidable but
      // capped at rAF rate (~60fps) instead of pointermove rate.
      const fakeEvent = {
        clientX: lastClientX,
        clientY: lastClientY,
      } as PointerEvent;
    ...
    const onMove = (me: PointerEvent) => {
      ...
      // Just update coords — the rAF loop handles drop target and scroll.
      lastClientX = me.clientX;
      lastClientY = me.clientY;

      if (dragStarted && rafId === null) {
        rafId = requestAnimationFrame(rafLoop);
      } else if (!dragStarted && me.pointerType !== 'mouse') {
        if (
