...
import { markProjectExported } from '../../lib/doc/db_helper';
import { updateProjectExportedAt } from '../../state/project_tree';
...
const LocalBackup: Component<Props> = (props) => {
  ...
  const markExported = async () => {
    ...
    const now = await markProjectExported(
      props.projectInfo.pjVerNodeId,
      deviceId(),
    );
    props.onExported?.(now);
  };
...
const BackupModal: Component<Props> = (props) => {
  const [exportedAt, setExportedAt] = createSignal(
    props.projectInfo?.exportedAt,
  );

  const handleExported = (at: string) => {
    setExportedAt(at);
    updateProjectExportedAt(at, deviceId());
  };
  ...
  return (
    <>
      <Show when={props.projectInfo}>
        ...
        <p class="opacity-60" style={{ 'font-size': 'var(--fs-xs)' }}>
          <Show
            when={exportedAt()}
            fallback={s('common.backup_never_exported')}
          >
            {s('common.backup_last_exported', {
              at: new Date(exportedAt()!).toLocaleString(),
            })}
          </Show>
        </p>
      </Show>
