...
export async function cleanupAndCompact(): Promise<CleanupResult> {
  ...
  for (const meta of allSheetMetas) {
    if (!projectIds.has(meta.projectId)) {
      console.log(
        `[cleanup] removing orphan sheetMeta ${meta.id} (project ${meta.projectId} gone)`,
      );
  ...
  const deltaCounts = new Map<string, number>();
  {
    ...
    while (cursor) {
      ...
      if (!validSheetIds.has(sheetId)) {
        console.log(`[cleanup] deleting delta for orphan sheet ${sheetId}`);
        await cursor.delete();
        result.orphanDeltas++;
      } else {
  ...
  for (const [sheetId, count] of deltaCounts) {
    if (count > 1) {
      console.log(`[cleanup] compacting sheet ${sheetId} (${count} deltas)`);
      await compactSheet(sheetId);
      result.compacted++;
    }
  }
